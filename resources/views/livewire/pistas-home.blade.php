        ...
        @foreach ($pistas as $pista)
            ...
            <article class="card grid grid-cols-6 shadow-lg rounded-xl">
                ...
                <div class="col-span-2 items-center content-center my-auto px-2">
                    @if ($pista->type=='pista')
                        <a href="{{route('ticket.pista.show', $pista)}}"><h1 class="text-xl text-gray-700 mb-2 py-2 font-bold text-center">{{Str::limit($pista->titulo,40)}}</h1>
                    @else
                        <a href="{{route('ticket.evento.show', $pista)}}"><h1 class="text-xl text-gray-700 mb-2 font-bold">{{Str::limit($pista->titulo,40)}}</h1>
                    @endif
                        @isset($pista->image)
                                @if ($pista->type=='pista')
